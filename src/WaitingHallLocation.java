...
public class WaitingHallLocation extends JPanel{
       Player player;
        ...
        WaitingHallLocation(Player player,MainFrame mainFrame)
        {
            this.player=player;
    ...
    public void paint(Graphics g)
    {
        ...
        if (player.playerRect.intersects(doorDownOne)) {player.currLocale=1;player.x=320;player.y=260;}
        if (player.playerRect.intersects(doorDownTwo)) {player.currLocale=1;player.x=570;player.y=260;}
        if (player.playerRect.intersects(doorRight)) {player.currLocale=4;player.x=205;player.y=400;}
        if (player.playerRect.intersects(doorUp)) {player.currLocale=5;player.x=425;player.y=645;}
