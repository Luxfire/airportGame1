...
public class Player {
    ...
    public void update()
    {
        ...
        if(moved)
        {
            if(moveSpeed==1)
            {
                if(down){y++;moveSpeed=0;}
                if (right){x++;moveSpeed=0;}
                if (left){x--;moveSpeed=0;}
                if (up) {y--;moveSpeed=0;}
            }
            if(animationSpeed==30) {spriteCounter++;animationSpeed=0;}
    ...
    public void drawPlayer(Graphics g)
    {
        ...
        if(!up&&!down&&!left&&!right)  g.drawImage(new ImageIcon(spriteDown.get(0)).getImage(),x,y,width,height,null);
