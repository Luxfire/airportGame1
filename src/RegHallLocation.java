...
public class RegHallLocation extends JPanel {
    ...
    RegHallLocation(Player player,MainFrame mainFrame)
    {
        this.player=player;
    ...
    public void paint(Graphics g)
    {
        ...
        player.playerRect = new Rectangle(player.x,player.y,player.width,player.height);
         ...
         if (player.playerRect.intersects(doorDownOne)) {player.currLocale=0; player.x=425; player.y=65;}
         if (player.playerRect.intersects(doorDownTwo)) {player.currLocale=2; player.x=425; player.y=310;}

        if (player.playerRect.intersects(doorUpOne)) {player.currLocale=3;player.x=320;player.y=615;}
        if (player.playerRect.intersects(doorUpTwo)) {player.currLocale=3;player.x=570;player.y=615;}
